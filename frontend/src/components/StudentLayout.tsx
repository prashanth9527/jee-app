...
// Menu structure with grouped sections for students
const menuSections = [
  {
    title: 'Main',
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      // {
      //   name: 'Chapterwise Practice',
      //   href: '/student/practice/content',
      //   icon: (
      //     <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    items: [
      {
        name: 'My AI Questions',
        href: '/student/my-ai-questions',
        icon: (
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    items: [
      ...
      //   href: '/student/ai/content-generator',
      //   icon: (
      //     <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      //       <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
      //     </svg>
      ...
      //   href: '/student/ai/adaptive-assessments',
      //   icon: (
      //     <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      //       <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
      //     </svg>
...
export default function StudentLayout({ children }: StudentLayoutProps) {
  ...
  useEffect(() => {
    // Use subscription status from AuthContext instead of making separate API call
    if (user?.subscriptionStatus) {
      setSubscriptionStatus({
        type: user.subscriptionStatus.hasValidSubscription ? 'premium' : 'trial',
        status: user.subscriptionStatus.hasValidSubscription ? 'active' : 'trial',
        isOnTrial: user.subscriptionStatus.isOnTrial,
        daysRemaining: user.subscriptionStatus.daysRemaining,
        needsSubscription: user.subscriptionStatus.needsSubscription
      });
      
      // Check if trial is expired and redirect to subscription page
      if (user.subscriptionStatus.needsSubscription && pathname !== '/student/subscriptions') {
        console.log('Trial expired, redirecting to subscription page');
        router.push('/student/subscriptions');
      }
    } else if (user?.role === 'STUDENT' && !user.needsProfileCompletion) {
      // Only refresh if we don't have subscription status yet
      refreshSubscriptionStatus();
    }
  }, [user, pathname, router, refreshSubscriptionStatus]);
