...
export default function EditPYQQuestionPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    subtopicId: '',
    options: [
      { id: '', text: '', isCorrect: false, order: 0 },
      { id: '', text: '', isCorrect: false, order: 1 },
      { id: '', text: '', isCorrect: false, order: 2 },
      { id: '', text: '', isCorrect: false, order: 3 }
    ],
    tagNames: [] as string[]
  });
  ...
  const loadQuestionData = async () => {
    try {
      setLoading(true);
      const [questionRes, subjectsRes, tagsRes] = await Promise.all([
        api.get(`/admin/pyq/${questionId}`),
      ...
      setFormData({
        ...
        subtopicId: questionData.subtopic?.id || '',
        ...
        tagNames: questionData.tags.map((t: any) => t.tag.name)
      });

      // Load topics and subtopics if subject/topic exists
      if (questionData.subject?.id) {
        await loadTopics(questionData.subject.id);
      }
      if (questionData.topic?.id) {
        await loadSubtopics(questionData.topic.id);
      }
    } catch (error) {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Failed to load question data. Please check the URL and try again.'
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/admin/pyq/questions/${questionId}`, questionData);
      
      Swal.fire({
        icon: 'success',
        title: 'Success!',
        text: 'PYQ question updated successfully',
      ...
      console.error('Error updating PYQ question:', error);
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: error?.response?.data?.message || 'Failed to update PYQ question'
      });
    } finally {
      setSubmitting(false);
    }
        ...
        <AdminLayout>
          <div className="min-h-screen bg-gray-50 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-4">The question you're looking for doesn't exist or has been deleted.</p>
              <button
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-3xl font-bold text-gray-900">Edit PYQ Question</h1>
                  <p className="mt-2 text-gray-600">Update the Previous Year Question details</p>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-lg p-8">
              <div className="space-y-8">
                ...
                <div className="flex justify-end space-x-4 pt-6 border-t border-gray-200">
                  ...
                  >
                    {submitting ? 'Updating...' : 'Update PYQ Question'}
                  </button>
                </div>
              </div>
            </form>
