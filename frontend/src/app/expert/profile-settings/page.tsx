...
export default function ExpertProfileSettingsPage() {
  ...
  // Redirect if not authenticated or not expert
  useEffect(() => {
    if (!user && !loading) {
      router.push('/login');
  ...
  return (
    <ExpertLayout>
      <DynamicHead
        ...
        description="Manage your expert profile settings and account information"
        keywords={`profile settings, account settings, expert profile, ${systemSettings?.siteKeywords || ''}`}
        canonicalUrl="/expert/profile-settings"
      />
