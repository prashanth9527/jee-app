...
export class StaticFilesController {
  ...
  @Get('latex/*')
  async viewLatex(@Param('0') filePath: string, @Res() res: Response) {
    try {
      console.log('LaTeX Request Debug Info:');
      console.log('- Received filePath:', filePath);
      ...
      // Construct the full file path
      const contentDir = join(process.cwd(), '..', 'content', 'latex');
      const fullPath = join(contentDir, decodedPath);
      console.log('- Content directory:', contentDir);
      console.log('- Full file path:', fullPath);
      ...
      console.log('File found, serving LaTeX...');

      // Extract filename from path for the Content-Disposition header
      const fileName = decodedPath.split('/').pop() || 'document.tex';

      // Set headers for LaTeX viewing
      ...
      throw new HttpException(
        {
          success: false,
          message: 'Failed to view LaTeX file',
          error: error.message
        },
        HttpStatus.INTERNAL_SERVER_ERROR
      );
  ...
  @Get('processed/*')
  async viewProcessed(@Param('0') filePath: string, @Res() res: Response) {
    try {
      console.log('Processed File Request Debug Info:');
      console.log('- Received filePath:', filePath);
      ...
      // Construct the full file path
      const contentDir = join(process.cwd(), '..', 'content', 'Processed');
      const fullPath = join(contentDir, decodedPath);
      console.log('- Content directory:', contentDir);
      console.log('- Full file path:', fullPath);
      ...
      if (!fs.existsSync(fullPath)) {
        console.error('File not found:', fullPath);
        throw new BadRequestException('Processed file not found');
      }
      
      console.log('File found, serving processed file...');

      // Extract filename from path for the Content-Disposition header
      const fileName = decodedPath.split('/').pop() || 'document.json';
      ...
      throw new HttpException(
        {
          success: false,
          message: 'Failed to view processed file',
          error: error.message
        },
        HttpStatus.INTERNAL_SERVER_ERROR
      );
